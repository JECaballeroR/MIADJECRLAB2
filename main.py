...
variables_numericas = ['PUNT_INGLES',
 'PUNT_MATEMATICAS',
 'PUNT_SOCIALES_CIUDADANAS',
 'PUNT_C_NATURALES',
 'PUNT_LECTURA_CRITICA',
 'PUNT_GLOBAL']
scatterx=col1.selectbox(label="Elija una de las variables numéricas para el eje X del Scatterplot", options=variables_numericas)
opciones_scatter2 = variables_numericas.copy()
opciones_scatter2.pop(variables_numericas.index(scatterx))
scattery=col1.selectbox(label="Elija una de las variables numéricas para el eje Y del Scatterplot", options=opciones_scatter2)


fig = px.scatter(data, x=scatterx, y=scattery,
             )
fig.update_traces(marker=dict(size=7,
                              line=dict(width=2,
                                        color='DarkSlateGrey')),
                  selector=dict(mode='markers'),opacity=0.69)
col1.plotly_chart(fig, use_container_width=True)


histx=col2.selectbox(label="Elije la variable categórica para observar su conteo y distribución entre puntajes ALTO y BAJO",
               options=[ 'COLE_BILINGUE','COLE_AREA_UBICACION', 'COLE_CALENDARIO',
       'COLE_CARACTER', 'COLE_GENERO', 'COLE_JORNADA', 'COLE_NATURALEZA',
       'COLE_DEPTO_UBICACION', 'ESTU_GENERO', 'FAMI_CUARTOSHOGAR',
...
st.markdown("""## Heatmap de PUNT_GLOBAL_CAT (clasificación del puntaje global) vs Otras variables categóricas. 

Las variables categóricas habilitadas son aquellas que llegaron al modelo final.

""")
heatmap_opcion=st.selectbox(label="Elige la opción de la variable a comparar con la clasificación del puntaje global",
             options= ['COLE_BILINGUE', 'COLE_CALENDARIO', 'COLE_JORNADA',
       'FAMI_EDUCACIONPADRE', 'FAMI_TIENEAUTOMOVIL'])

st.plotly_chart(plot_heatmap(data,'PUNT_GLOBAL_CAT', heatmap_opcion), use_container_width=True)
